...
const MetricsIndex = (props) => {
  ...
  const onReportFetched = (result) => {
    setReportItems(result.getExamsReportData.data)
  }

  const onReportError = () => {
    alertError(formatMessage({ id: 'metrics_error' }))
  }
      ...
      <Card className='mx-auto shadow mb-3 bg-white rounded'>
        ...
        <CardBody>
          <div className='row'>
            ...
            <div className='col-md-4 col-xs-12 text-center'>
              <Reports data={reportItems} disabled={fetchingReportData || !reportItems.length === 0} />
            </div>
          </div>
